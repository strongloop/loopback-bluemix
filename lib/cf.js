 ...
 * @param cb
 */
function getApps(accessToken, cb) {
  getResource('/v2/apps', accessToken, function(err, res) {
    if (err) return cb(err);
    var apps = res.body;
 ...
 * @param cb
 */
function getServices(parent, accessToken, cb) {
 ...
 * @param cb
 */
function getDataServices(parent, accessToken, cb) {
  getServices(parent, accessToken, function(err, services) {
    if (err) return cb(err);
    var dataServices = services.filter(function(s) {
      return Array.isArray(s.entity.tags) &&
        s.entity.tags.indexOf('data_management') !== -1;
    });
    async.each(dataServices, function(s, done) {
      getServicePlans(s, accessToken, function(err, plans) {
 ...
 * @param cb
 */
function getServicePlans(parent, accessToken, cb) {
 ...
 * @param cb
 */
function getDataServices(accessToken, cb) {
  invokeResource('/v2/services', accessToken, function(err, res, body) {
    var getDataServiceDetails = {};
    body.resources.forEach(function(resource) {
      var label = resource.entity.label;
      if (bluemixSupportedServiceLabels.indexOf(label) > -1) {
        getDataServiceDetails[label] = {};
        var service = getDataServiceDetails[label];
        service.guid = resource.metadata.guid;
        service.url = resource.metadata.url;
        service.plansUrl = resource.entity['service_plans_url'];
      }
    });
    cb(null, getDataServiceDetails);
  });
}

/**
 * Get data service plans
 * @param accessToken
 * @param servicePlansUrl
 * @param cb
 */
function getServicePlans(accessToken, servicePlansUrl, cb) {
  invokeResource(servicePlansUrl, accessToken, function(err, res, body) {
    if (err) return cb(err);
    cb(null, body.resources);
  });
}
 ...
 * @param cb
 */
function bindService(accessToken, details, cb) {
  var options = {
    method: 'post',
    body: details,
  };
 ...
 * @param cb
 */
function provisionService(accessToken, details, cb) {
  var options = {
    method: 'post',
    body: details,
  };
