...
function provisionService(datasource, globalize) {
  ...
  return datasource.prompt(prompts).then(function(answers) {
    var details = {};
    details.name = answers.serviceName;
    details['service_plan_guid'] = servicePlansGUIDs[answers.servicePlan];
    details['space_guid'] = cfConfig.space.guid;
    cf.provisionService(cfConfig.accessToken, details, function(err) {
      if (err) return done(err);
    });
  }.bind(datasource));
}
